...
export default function useReadMore(text: string, config?: Config): {
  firstPart: string, secondPart: string
} {
  if (!config) config = {}
  const { treshold = 50, firstPartMaxLength = -1
  } = config

  if (text.length === 0) throw new Error("Text length should be greater than 0")
  if (treshold <= 0 || treshold >= 100) throw new Error("Treshold should be greater than 0 and less than 100")

  const textArray = text.split(" ")

  const [firstPart, setFirstPart
  ] =
    useState<string>(textArray
      .slice(0, Math.floor(textArray.length * treshold / 100)).join(" "))

  const [secondPart, setSecondPart
  ] = useState<string>(text.split(" ").slice(Math.floor(textArray.length * treshold / 100), textArray.length).join(" "))
  ...
  useEffect(() => {
    if (firstPartMaxLength !== -1) {
      ...
      setSecondPart(textArray.slice(firstPartMaxLength, textArray.length).join(" "))
    }
  },
    [])
